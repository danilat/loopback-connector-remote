...
function createProxyMethod(Model, remotes, remoteMethod) {
  ...
  function remoteMethodProxy() {
    ...
    if (remoteMethod.isStatic) {
      return remotes.invoke(remoteMethod.stringName, args, function(err, resp){
        parseStringsToDates(resp)
        callback(err, resp)
      });
    }

    var ctorArgs = [this.id];
    return remotes.invoke(remoteMethod.stringName, ctorArgs, args, , function(err, resp){
      parseStringsToDates(resp)
      callback(err, resp)
    });
  }
...
function parseStringsToDates(resp){
  if(Array.isArray(resp)){
    for(key in resp){
      parseStringsToDates(resp[key]);
    }
  }else if(typeof resp === 'object'){
    for (property in resp) {
      if(typeof resp[property] === 'string'){
        var possibleDate = new Date(resp[property]);
        var isValid = !isNaN(possibleDate.getTime());
        if(isValid){
          resp[property] = possibleDate;
        }
      }
    }
  }
